...
#### Duplicated rows with the highest id
####
####
cursor.execute('''
               SELECT t.*
...
for col in column_names_txn:
    ...
    list_empty_txn.append(empty_count)

list_empty_null_txn = list_empty_txn + list_empty_txn

#### Handle empty AccountIds in the transactions table
####
####
list_empty_accid = []
if list_empty_null_txn[1] != 0:
    query1 = f'SELECT TransactionId FROM transactions WHERE AccountId = ""'
    cursor.execute(query1)
    list_empty_accid = [row[0] for row in cursor.fetchall()]
    print(list_empty_accid)
# for aid in list_empty_accid:
#   query = f'SELECT TransactionId FROM transactions WHERE AccountId = ""
####
####
####
...
for col in column_names_acc:
    ...
    cursor.execute(query)
    empty_count = cursor.fetchone()[0]
    list_empty_acc.append(empty_count)
print(list_empty_acc)
